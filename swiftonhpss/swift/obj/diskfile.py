...
class DiskFile(object):
    ...
    def open(self):
        ...
        try:
            self._logger.info("DiskFile: Opening %s" % self._data_file)
            ...
            raise
        try:
            self._logger.info("DiskFile: Reading metadata")
    ...
    def create(self, hpss_hints):
        ...
        try:
            self._logger.info("DiskFile: Creating directories for %s" %
                              self._container_path)
        ...
        self._logger.info("DiskFile: creating directories in obj name %s" %
                          self._obj)
        object_dirs = os.path.split(self._obj)[0]
        self._logger.info("object_dirs: %s" % repr(object_dirs))
        self._logger.info("data_dir: %s" % self._put_datadir)
        ...
        while True:
            ...
            self._logger.info("DiskFile: Creating temporary file %s" %
                              tmppath)

            try:
                # TODO: figure out how to create hints struct, pass it into open
                self._logger.info("DiskFile: creating file")
                fd = do_open(tmppath,
                             hpss.O_WRONLY | hpss.O_CREAT | hpss.O_EXCL)
                self._logger.info("DiskFile: setting COS")
                if hpss_hints['cos']:
                ...
                if gerr.errno in (errno.ENOSPC, errno.EDQUOT):
                    ...
                    self._logger.info("DiskFile: no space")
                    raise DiskFileNoSpace()
                if gerr.errno == errno.EACCES:
                    self._logger.info("DiskFile: permission denied")
                    raise DiskFileNoSpace()
                if gerr.errno == errno.ENOTDIR:
                    self._logger.info("DiskFile: not a directory")
                ...
                if gerr.errno not in (errno.ENOENT, errno.EEXIST, errno.EIO):
                    ...
                    self._logger.info("DiskFile: unknown error %s" % gerr.errno)
                    raise
                ...
                if gerr.errno == errno.EEXIST:
                    self._logger.info("DiskFile: file exists already")
        ...
        self._logger.info("DiskFile: created file")

        try:
            #if size is not None and size > 0:
            #    try:
            #        hpss_utils.preallocate(fd, size)
            #    except OSError as err:
